{ config, lib, pkgs, ... }:
let
  inherit (lib) types mkIf mkMerge mkOption;
  nixosConfig = config;
in
{
  options.my-config = {
    git = {
      enable = mkOption {
        description = "Enable my customized git";
        type = types.bool;
        default = false;
      };
      signing = {
        enable = mkOption {
          description = "Enables signing commits and tags";
          type = types.bool;
          default = false;
        };
      };
      profiles = {
        work = {
          enable = mkOption {
            description = "Enables work profile";
            type = types.bool;
            default = false;
          };
          signingKey = mkOption {
            description = "Work signing key";
            type = types.str;
          };
        };
        spbpu = {
          enable = mkOption {
            description = "Enables spbpu profile";
            type = types.bool;
            default = false;
          };
          signingKey = mkOption {
            description = "SPbPU signing key";
            type = types.str;
          };
        };
        personal = {
          signingKey = mkOption {
            description = "Personal signing key";
            type = types.str;
          };
        };
      };
      corporateUrls = {
        enable = mkOption {
          description = "Enable corporate git URL rewrites via sops-encrypted include";
          type = types.bool;
          default = false;
        };
        secretsFile = mkOption {
          description = ''
            Path to sops-encrypted YAML file.
          '';
          type = types.path;
          default = ../../secrets/hosts.yaml;
        };
      };
    };
  };

  config = mkIf nixosConfig.my-config.git.enable {
    home-manager.users.wrbbz =
      # Function form so we can reference config.sops.{placeholder,templates}
      { config, lib, ... }:
      {
        home.packages = with pkgs; [
          gh
          git-cliff
          glab
        ];

        sops.secrets."excorp_gitlab" = mkIf (
          nixosConfig.my-config.git.corporateUrls.enable
          && nixosConfig.my-config.git.profiles.work.enable
        ) {
          sopsFile = nixosConfig.my-config.git.corporateUrls.secretsFile;
          key = "excorp_gitlab";
        };

        sops.secrets."spbpu_gitlab" = mkIf (
          nixosConfig.my-config.git.corporateUrls.enable
          && nixosConfig.my-config.git.profiles.spbpu.enable
        ) {
          sopsFile = nixosConfig.my-config.git.corporateUrls.secretsFile;
          key = "spbpu_gitlab";
        };

        sops.templates."git-corporate-urls" = mkIf nixosConfig.my-config.git.corporateUrls.enable {
          content =
            lib.optionalString nixosConfig.my-config.git.profiles.work.enable ''
              [url "exit:"]
                  insteadOf = https://${config.sops.placeholder."excorp_gitlab"}/
            ''
            + lib.optionalString nixosConfig.my-config.git.profiles.spbpu.enable ''
              [url "g215:"]
                  insteadOf = https://${config.sops.placeholder."spbpu_gitlab"}/
            '';
        };

        programs = {
          git = {
            enable = true;
            signing = mkIf nixosConfig.my-config.git.signing.enable {
              key = nixosConfig.my-config.git.profiles.personal.signingKey;
              signByDefault = true;
            };
            settings = {
              user = {
                email = "me@wrb.bz";
                name = "Arsenii Zorin";
              };
              alias = {
                lg = "log --graph --abbrev-commit --decorate --format=format:'%C(bold blue)%h%C(reset) - %C(bold cyan)%aD%C(reset) %C(bold green)(%ar)%C(reset)%C(bold yellow)%d%C(reset)%n%C(white)%s%C(reset) %C(dim green)- %an%C(reset)' --all";
              };
              init = {
                defaultBranch = "main";
              };
              safe.directory = "/etc/nixos";
              pull = {
                rebase = true;
              };
              commit = {
                template = "~/.config/git/message";
              };
              core.hooksPath = "~/.config/git/hooks";
            };
            ignores = [
              "*.swp"
              ".envrc"
              ".npmrc"
              ".werf*"
              ".netrc"
              ".DS_Store"
            ];
            includes = [
              (mkIf nixosConfig.my-config.git.profiles.work.enable {
                condition = "gitdir:~/work/excorp/";
                contents = {
                  user = mkMerge [
                    {
                      email = "a.zorin@cs.money";
                    }
                    (mkIf nixosConfig.my-config.git.signing.enable {
                      signingkey = nixosConfig.my-config.git.profiles.work.signingKey;
                    })
                  ];
                };
              })
              (mkIf nixosConfig.my-config.git.profiles.spbpu.enable {
                condition = "gitdir:~/work/spbpu/";
                contents = {
                  user = mkMerge [
                    {
                      email = "arseny.zorin@spbpu.com";
                    }
                    (mkIf nixosConfig.my-config.git.signing.enable {
                      signingkey = nixosConfig.my-config.git.profiles.spbpu.signingKey;
                    })
                  ];
                };
              })
              (mkIf nixosConfig.my-config.git.corporateUrls.enable {
                path = config.sops.templates."git-corporate-urls".path;
              })
            ];
          };
          delta = {
            enable = true;
            enableGitIntegration = true;
          };
        };
        home.file = {
          ".config/git/message" = { source = ./message; };
          ".config/git/hooks/commit-msg" = {
            executable = true;
            source = ./hooks/commit-msg;
          };
        };
      };
  };
}
